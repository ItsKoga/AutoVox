...
class AutoVoice(commands.Cog):
    ...
    async def on_voice_state_update(self, member, before, after):
        ...
        guild = member.guild
        createChannel = database.execute_read_query(f"SELECT * FROM settings WHERE guild_id = {member.guild.id} AND setting_name = 'create_channel'")
        if not createChannel:
            return
        createChannel = guild.get_channel(int(createChannel[0][2]))
